...
import { approveDraftInputSchema, dismissDraftInputSchema, triggerAnalysisInputSchema } from "@shared/types";
...
export function createSupportAnalysisRouter(dispatcher: WorkflowDispatcher) {
  return router({
    ...
    dismissDraft: workspaceProcedure
      .input(dismissDraftInputSchema)
      .mutation(({ ctx, input }) =>
        dismissSupportDraft({
          ...input,
          workspaceId: ctx.workspaceId,
    ...
    getLatestAnalysis: workspaceProcedure
      .input(triggerAnalysisInputSchema)
      .query(({ ctx, input }) =>
        getLatestAnalysis(input.conversationId, ctx.workspaceId)
      ),
  });
}
