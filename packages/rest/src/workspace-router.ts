      ...
      if (plan?.planCatalog) {
        ...
        if (maxSeats > 0) {
          ...
          if (activeMemberCount >= maxSeats) {
            throw new TRPCError({
              code: "FORBIDDEN",
              message: `Seat limit reached (${maxSeats}). Upgrade your plan to add more team members.`,
            });
          }
        }
      }
